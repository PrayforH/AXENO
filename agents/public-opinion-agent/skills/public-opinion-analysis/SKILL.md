...
2. 选择最窄的数据路径：用户材料 → 已注册专用舆情 MCP → Tavily 公网补充。不要用公网样本冒充专用平台或全网数据。
3. 需要构造条件、查询专用数据或分析热搜时，读取 `.claude/skills/public-opinion-analysis/references/query-contract.md` 并按其规范执行。
...
可将工作区内材料的只读核验、观点聚类和行业背景分析分别委派给声明的 Sub Agent。每次委派写明文件范围、问题和返回格式。Lead Agent 负责所有联网/专用数据查询、证据交叉核验和最终风险判断。
