...
- 外部检索必须由主 Agent 直接调用注册的只读 Tavily 工具。不得把网页搜索、联网验证或 URL 抽取委派给任何子 Agent。
- `helper-agent` 只用于读取和归纳已经存在于工作区的材料；委派时必须明确文件范围，不得要求它联网。
- 如果 Tavily 调用失败或工具不可用，明确说明联网检索未完成及原因，不得让子 Agent 代替搜索，也不得把模型记忆写成最新事实。
