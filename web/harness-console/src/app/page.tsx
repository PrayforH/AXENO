...
export default function Home() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const restored: AguiEvent[] = [];
        await client.events(runId, undefined, (id, event) => {
          setLastEventId(id);
          restored.push(event);
        });
        setEvents(restored);
        setArtifacts(await client.artifacts(runId));
      } catch {
  ...
  async function decideApproval(approvalId: string, decision: "approved" | "rejected") {
    try {
      await client.approve(approvalId, decision);
      await client.events(runId, lastEventId, (id, event) => {
        setLastEventId(id);
