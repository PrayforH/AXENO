...
class PostgresMcpCredentialRepository:
    ...
    async def get(self, tenant_id: str, reference: str) -> StoredMcpCredential | None:
        async with self._sessions() as session:
            row = await session.get(McpCredentialRow, (tenant_id, reference))
        ...
        async with self._sessions() as session:
            rows = (
                await session.scalars(
                    ...
                    .where(McpCredentialRow.tenant_id == tenant_id)
    ...
    async def upsert(self, value: StoredMcpCredential) -> StoredMcpCredential:
        async with self._sessions() as session:
            row = await session.get(McpCredentialRow, (value.tenant_id, value.reference))
            if row is None:
