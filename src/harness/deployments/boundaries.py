...
def enforce_runtime_environment(
    ...
    if denied_routes:
        raise ConflictError(
            "Session Environment snapshot denies model routes: "
            + ", ".join(denied_routes)
        )
    mcp_references = {
        tool.mcp for tool in agent.manifest.spec.tools if tool.mcp is not None
    }
    denied_mcp = sorted(mcp_references - set(policy.allowed_mcp_references))
    if denied_mcp:
        raise ConflictError(
            "Session Environment snapshot denies MCP resources: "
            + ", ".join(denied_mcp)
        )
