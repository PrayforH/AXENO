...
class InMemoryEnvironmentRepository:
    def __init__(self) -> None:
        self._items: dict[tuple[str, str, EnvironmentName], Environment] = {}
        self._lock = asyncio.Lock()

    async def add(self, environment: Environment) -> None:
        key = (environment.tenant_id, environment.agent_name, environment.name)
    ...
    async def get(self, tenant_id: str, agent_name: str, name: EnvironmentName) -> Environment:
        try:
            return self._items[(tenant_id, agent_name, name)]
    ...
    async def list_for_agent(self, tenant_id: str, agent_name: str) -> list[Environment]:
        return sorted(
            (
                item
                for (t, agent, _name), item in self._items.items()
                if t == tenant_id and agent == agent_name
            ),
            key=lambda item: item.name.value,
        )

    async def compare_and_set(self, expected_revision: int, updated: Environment) -> bool:
        key = (updated.tenant_id, updated.agent_name, updated.name)
        async with self._lock:
...
class InMemoryDeploymentRepository:
    ...
    async def list_snapshots(self, tenant_id: str, agent_name: str) -> list[DeploymentSnapshot]:
        return sorted(
            (
                ...
                if t == tenant_id and item.agent_name == agent_name
            ),
    ...
    async def add(self, deployment: Deployment) -> None:
        ...
        idem = (deployment.tenant_id, deployment.idempotency_key)
        async with self._lock:
    ...
    async def find_by_idempotency(self, tenant_id: str, key: str) -> Deployment | None:
        deployment_id = self._idempotency.get((tenant_id, key))
